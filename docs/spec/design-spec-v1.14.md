...
**Current product version:** `1.2.0`; session-profile and managed Windows service feature release
...
4. Linux, macOS, and Unix output must explicitly state that managed `$sql-context-pack setup`,
   repair, update, and uninstall are not supported in this release because they manage a Windows
   Service through PowerShell.
5. Linux, macOS, and Unix output routes owners to the explicit host-Python/development workflow:
   verify `python3`, run the POSIX preflight, install the package into the selected host
   interpreter user site when appropriate, configure/test profiles, and start the loopback service
...
4. Native install makes the Skill available; `$sql-context-pack setup` then explains and deploys
   the owner package and automatic loopback Windows Service from the installed plugin cache. The
...
8. Uninstall must be initiated with `$sql-context-pack uninstall` while the Skill is still
   installed. It removes the Windows Service and owner package before the dedicated native
