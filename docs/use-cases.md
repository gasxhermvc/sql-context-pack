...
|---|---|---|
| “สร้าง SQL context ที่ `./sql-context`” | `ask`; present all preliminary groups, then analyze all objects after selection. | Direct category folders plus indexes/reports. |
...
| “สร้างเฉพาะ `um` และ `content` ที่ `./docs/db`” | `selected`; full extraction remains unrestricted; excluded connected objects remain boundary metadata. | Final `um/` and `content/` SQL only. |
