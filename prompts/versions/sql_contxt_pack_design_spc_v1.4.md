...
**Initial product version:** `1.0.0` from the first implementation commit; no prerelease suffix  
...
### Revision v1.4 — Cut-off

This cut-off revision retains every accepted v1.3 behavior, incorporates the append-only raw change log, and resolves the final review blockers before product `1.0.0` implementation begins:
...
- every SQLFluff command uses the explicitly selected managed-runtime interpreter and each export pins its runtime,
...
- the product starts at `1.0.0`; corrective iterations bump patch (`1.0.1`, `1.0.2`, …) and use a non-circular two-phase gate without a final version transition.
...
```json
{
  "runtime_id": "rt_sqlfluff_X_Y_Z_01J...",
  "interpreter_path": "<absolute-managed-runtime-python>",
  "python_version": "3.x",
...
The active-runtime pointer resolves to an immutable `runtime_id` plus an absolute interpreter path. Production orchestration must never resolve SQLFluff through the server's ambient `python` executable.

### 9.3 Ensure algorithm

```text
1. Resolve the active managed-runtime pointer and its absolute interpreter path.
2. Check SQLFluff metadata and run `<selected-runtime-python> -m sqlfluff version` inside that runtime.
...
6. New export jobs pin the new immutable `runtime_id`; already-running jobs continue using the runtime they pinned at job creation.
7. Keep one previous version for rollback and retain any older runtime still pinned by an active job.
8. Remove only inactive, unpinned older runtimes.
9. Do not modify the target project.
```
...
```bash
<selected-runtime-python> -m sqlfluff format \
...
3. Run:
   <job-pinned-runtime-python> -m sqlfluff parse --dialect <dialect> --templater raw <temp-file>
4. If parse fails:
   a. do not format;
...
5. If parse succeeds:
   a. run `<job-pinned-runtime-python> -m sqlfluff format` on the temporary file;
   b. run parse again through the same job-pinned interpreter;
...
Version 1 exposes no Boolean switch that can disable SQLFluff or sample appending. The application always formats final-materialization SQL through the job-pinned runtime and appends sanitized samples/shortage metadata according to catalog policy. A compatibility client that sends `sqlfluff:false` or `append_samples:false` receives `400 MANDATORY_EXPORT_STAGE_DISABLED`; exclude rules come from the versioned server policy, not arbitrary export input.
...
{
  "items": [
    {"export_id":"exp_01J...","catalog_id":"cat_01J...","status":"completed_with_warnings","request_fingerprint":"sha256:export-request-...","object_batch_fingerprint":"sha256:ordered-object-ids-...","runtime_id":"rt_sqlfluff_X_Y_Z_01J...","size_bytes":481002,"sha256":"sha256:bundle-...","manifest_sha256":"sha256:manifest-...","output_format_version":"1","expires_at":"2026-07-19T12:30:00+07:00"}
  ],
  "page": {"limit":100,"returned":1,"next_cursor":null}
}
```

The export `request_fingerprint` covers canonical catalog ID, ordered object-batch fingerprint, mandatory format/sample policy versions, pinned runtime ID, and output format version. Resume only an exact match.
...
For a completed status, `size_bytes`, bundle `sha256`, `manifest_sha256`, `runtime_id`, and `output_format_version` are immutable. HTTP and `sqlctx_get_export_status` return the same normalized values. `sqlctx export fetch` requires them before download, enforces the declared size while streaming, and rechecks both bundle and manifest hashes afterward.
...
The update endpoint must enforce the Section 12.4 request-bound owner approval or direct owner-control CLI authentication; an agent bearer alone receives `403 APPROVAL_REQUIRED`.
...
| `GET /exports/{export_id}` | path only | `ExportStatus` | pollable; completed state includes immutable size/hashes/runtime/format version |
...
| `POST /tooling/sqlfluff/update` | `SqlFluffUpdateRequest` | `SqlFluffUpdateResult` | request-bound owner approval; explicit update only |
...
1. The harness authenticates with an `agent`-scoped token. That token alone cannot delete jobs, persist classification resolutions/overrides, update SQLFluff, enable remote access, or weaken masking.
...
| `sqlctx_get_export_status` | `ExportIdRequest` | `ExportStatus` | returns immutable size/hashes/runtime/format version and resource links, not large content |
...
| `sqlctx_sqlfluff_update` | `SqlFluffUpdateRequest` | `SqlFluffUpdateResult` | request-bound owner approval; explicit update only |
...
|---|---|---|
| specification version | `1.4` | cut-off version of this design/prompt document |
| product/package/Skill version | SemVer, initially `1.0.0` | server, CLI, canonical Skill, and three harness packages developed and released together; no prerelease suffix |
...
1. Initialize every product surface at `1.0.0` in Chunk 0/1. Do not use `-dev.N`, `-rc`, or another prerelease suffix.
...
3. If testing or review discovers a defect that requires a repository change, increment patch once for that corrective iteration (`1.0.1`, `1.0.2`, and so on), add the fix to `CHANGELOG.md`, synchronize every version surface, and restart the gate. Aggregate related file edits into that one correction version.
...
- Commit this specification byte-for-byte as `docs/spec/design-spec-v1.4.md` and record its SHA-256 in `docs/spec/design-spec-v1.4.sha256`.
- Treat v1.4 as the only implementation source of truth; raw and v1.1–v1.3 remain archive/traceability inputs only.
- Every fresh implementation session reads `docs/implementation-state.md`, the immutable invariants, and only the routed v1.4 sections needed for its chunk.
...
- Install/update two SQLFluff managed runtimes while an export is active; confirm the active export remains on its pinned interpreter and the next export uses the new absolute interpreter, regardless of ambient Python contents.
...
This is a fresh Chunk 0 session. Read `docs/spec/design-spec-v1.4.md` Sections 1–4, 19.10–19.13, and 20 before changing implementation files. If the file is not present yet, copy the supplied v1.4 artifact byte-for-byte into that path, record its SHA-256, and then read those routed sections. Do not summarize or regenerate the authoritative specification.
...
- Start every product surface at `1.0.0` with no prerelease suffix. Planned v1 chunks remain on `1.0.0`; a corrective iteration after a failed review/gate bumps patch and restarts the gate under Section 19.10.
...
1. `docs/spec/design-spec-v1.4.md` as the byte-for-byte authoritative artifact plus `docs/spec/design-spec-v1.4.sha256`,
...
10. `CHANGELOG.md`,
11. a central version source initialized at `1.0.0` with no prerelease suffix.

Derived documents must link to exact authoritative sections and add implementation decisions only where necessary. They must not become rewritten substitute contracts. Before stopping, record completed Chunk 0 in `CHANGELOG.md` and `docs/implementation-state.md`; keep `1.0.0` unless Section 19.10 requires a corrective patch bump.
...
This is a fresh Chunk 1 session. Read `docs/implementation-state.md`, `CHANGELOG.md`, and `docs/spec/design-spec-v1.4.md` Sections 1.3, 3, 6, 11.20, 12, 19.1, and 19.10–19.13. Do not reinterpret or weaken the immutable contract.
...
This is a fresh Chunk 2 session. Read `docs/implementation-state.md`, `CHANGELOG.md`, and `docs/spec/design-spec-v1.4.md` Sections 4–5, 8–9, and 17.5–17.6. Do not modify the immutable product scope.
...
- resolve the selected managed-runtime interpreter and run `<selected-runtime-python> -m sqlfluff version`,
...
- self-test,
- atomically switch,
- keep one rollback version and every runtime pinned by an active export.
...
3. parse with `<job-pinned-runtime-python> -m sqlfluff parse`,
4. if parse fails, preserve cleaned original and continue,
5. if parse succeeds, format with:
   `<job-pinned-runtime-python> -m sqlfluff format --exclude-rules "CP02,LT01,RF06" --dialect <dialect> --templater raw <file>`
...
- ambient Python SQLFluff is ignored; active jobs stay on their pinned managed interpreter across an update.
...
This work is deliberately split into fresh sessions. Each sub-chunk reads `docs/implementation-state.md`, `CHANGELOG.md`, and `docs/spec/design-spec-v1.4.md` Sections 6–8, 10, and 17:
...
This is a fresh Chunk 4 session. Read `docs/implementation-state.md`, `CHANGELOG.md`, and `docs/spec/design-spec-v1.4.md` Sections 10, 13–16, and 19.3–19.6.
...
This is a fresh Chunk 5 session. Read `docs/implementation-state.md`, `CHANGELOG.md`, and `docs/spec/design-spec-v1.4.md` Sections 4.5, 11–12, and 17.3–17.6. The endpoint/tool lists below are only an index; implement exact request, response, pagination, idempotency, authorization, integrity, and example contracts from Sections 11–12. Do not invent schemas from this abbreviated list.
...
- Completed `ExportStatus` returns immutable `size_bytes`, bundle `sha256`, `manifest_sha256`, `runtime_id`, and `output_format_version` over both interfaces.
- Rediscovery descriptors include exact normalized request/batch fingerprints; never resume from profile/status alone.
- Privileged delete, authoritative resolution, and SQLFluff update require a request-bound one-time owner approval; an agent token alone receives `APPROVAL_REQUIRED`.
...
This is a fresh Chunk 6 session. Read `docs/implementation-state.md`, `CHANGELOG.md`, and `docs/spec/design-spec-v1.4.md` Sections 13, 15, 17–18, and 21.
...
Split this work across fresh sessions, each reading `docs/implementation-state.md`, `CHANGELOG.md`, and `docs/spec/design-spec-v1.4.md` Sections 1.3, 3.4–3.5, 19, and 21–23:
...
Complete, test, document, and update implementation state for 7A. Keep the current release version (initially `1.0.0`) unless a failed review/gate requires the corrective patch bump in Section 19.10. Chunk 7B runs the two-phase gate without adding a prerelease suffix or changing version merely because the gate passed.
...
- Start at `1.0.0` with no prerelease suffix and keep planned v1 implementation on that version.
...
28. Every product surface starts at `1.0.0` without a prerelease suffix; corrective iterations bump patch and restart the two-phase gate, while passing the gate does not change version.
...
39. `docs/spec/design-spec-v1.4.md` matches the authoritative artifact hash and fresh sessions use routed sections plus implementation state.
...
41. Every SQLFluff parse/format/version command uses the explicitly selected managed-runtime interpreter; active exports remain pinned across updates.
...
45. Completed export status exposes immutable size, bundle hash, manifest hash, runtime ID, and output format version over HTTP and MCP.
...
47. An agent token alone cannot delete, confirm classifications, update SQLFluff, enable remote mode, or weaken masking; privileged actions consume a matching one-time owner approval.
...
| Original raw requirement | Historical assessment through v1.2 | Current v1.4 authoritative coverage |
|---|---|---|
...
| Model updates changelog and increases version | v1.2 required a release bump for every repository edit, which caused build-version drift | Sections 19.10, Chunk 0–7, and Definition of Done item 28 start at `1.0.0`; corrective iterations bump patch and rerun the gate |
...
| Appended raw requirement | v1.2 finding | Current v1.4 closure |
|---|---|---|
...
| 17 build/release versions | every-edit release bump could drift before release | Section 19.10 and Chunks 0–7 use immediate `1.0.0`, correction-level patch bumps, and a two-phase same-version gate |
| 18 authoritative spec routing | Chunk 0 paraphrase could drift from the design contract | Sections 1.3, 19.12, 20 require byte-for-byte v1.4, hash, section routes, and implementation state |
...
| 20 source precedence | old versions could conflict during implementation | Sections 19.12 and 20 make v1.4 the only implementation source of truth |
...
# 27. Resolved Trade-offs for v1.4

| Decision | Selected v1.4 behavior | Rejected alternative and reason |
|---|---|---|
...
| implementation context | byte-for-byte v1.4 in repo, routed fresh sessions | a single long context or agent-written contract summary invites drift |
| product version | use `1.0.0` from the first implementation commit; bump patch for corrective iterations | prerelease suffixes were explicitly rejected for this build-and-release workflow |
...
| Final review blocker | Resolution in v1.4 |
|---|---|
...
| SQLFluff update could select one runtime but execute another | resolve an absolute managed interpreter and pin immutable runtime ID per export |
...
| completed export status lacked integrity data | return immutable size, bundle/manifest hashes, runtime ID, and output format version over HTTP/MCP |
...
```text
Authoritative specification: 1.4
